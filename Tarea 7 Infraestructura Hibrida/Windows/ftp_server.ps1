...
function New-FtpSite {
    ...
    $siteNode = $config.configuration.'system.applicationHost'.sites.site | Where-Object { $_.name -eq $CFG_FTP_SITE }
    ...
    $sitio2 = $cfg2.configuration.'system.applicationHost'.sites.site | Where-Object { $_.name -eq $CFG_FTP_SITE }
...
function Invoke-PopularRepositorio {
    ...
    Get-ChildItem -Path $CFG_FTP_REPO -Recurse -File | ForEach-Object {
...
function Invoke-FtpServerSetup {
    while ($true) {
        ...
        Write-Host "   CONFIGURACION SERVIDOR FTP (IIS FTP)               " -ForegroundColor Green
        Write-Host "======================================================" -ForegroundColor Cyan
        ...
        Write-Host "Selecciona una opcion [0-6]: " -ForegroundColor Green -NoNewline
        $opc = Read-Host
