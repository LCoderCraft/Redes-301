...
function Set-FirewallRule {
    ...
    New-NetFirewallRule -DisplayName $Nombre -Direction Inbound `
        -Protocol TCP -LocalPort $Puerto -Action Allow -Profile Any -ErrorAction SilentlyContinue | Out-Null
...
<body>
  <div class="card">
    ...
    <p>Versi&oacute;n:  <span>$Version</span></p>
    <p>Puerto:   <span>$Puerto</span></p>
    <p>Protocolo: <span>$proto</span></p>
    <hr style="border-color:#30363d; margin:20px 0;">
    <small>Aprovisionamiento Automatizado - Pr&aacute;ctica 7</small>
  </div>
</body>
</html>
"@ 
    
    $html | Set-Content "$Webroot\index.html" -Encoding UTF8 -Force
}
...
function New-SslCert {
    ...
    try {
        Write-Host "  [INFO] Extrayendo clave privada nativamente..." -ForegroundColor Cyan
        $rsa = [System.Security.Cryptography.X509Certificates.RSACertificateExtensions]::GetRSAPrivateKey($cert)
        
        $format = [System.Security.Cryptography.CngKeyBlobFormat]::Pkcs8PrivateBlob
        $keyBytes = $rsa.Key.Export($format)
        
        $b64Key = [Convert]::ToBase64String($keyBytes, [System.Base64FormattingOptions]::InsertLineBreaks)
        $pemKey = "-----BEGIN PRIVATE KEY-----`n$b64Key`n-----END PRIVATE KEY-----"
        $pemKey | Set-Content $CFG_SSL_KEY -Encoding ASCII
        Write-Host "  [OK] Clave privada exportada exitosamente." -ForegroundColor Green
    } catch {
        Write-Host "  [ERROR] Fallo al extraer la clave nativamente: $_" -ForegroundColor Red
        return $false
    }
...
function Install-Apache {
    ...
    if (-not (Test-Path "$src\bin\httpd.exe")) {
        $found = Get-ChildItem $extractTemp -Recurse -Filter "httpd.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
function Install-Nginx {
    ...
    Write-Host "  [INFO] Iniciando nginx en segundo plano..." -ForegroundColor Cyan
    Start-Process "$dest\nginx.exe" -WorkingDirectory $dest
    Start-Sleep -Seconds 2
    
    $proc = Get-Process -Name "nginx" -ErrorAction SilentlyContinue
    if ($proc) { 
        Write-Host "  [OK] nginx corriendo exitosamente (PID $($proc[0].Id))." -ForegroundColor Green 
    } else { 
        Write-Host "  [ERROR] nginx no pudo iniciarse. Revisa la sintaxis del config:" -ForegroundColor Red 
        & "$dest\nginx.exe" -t
    }
...
# ============================================================
#  IIS (Modo Seguro + Corrección Error 401)
# ============================================================
...
function Install-IIS {
    ...
    Write-Host "`n======================================================" -ForegroundColor Cyan
    Write-Host "  Instalando IIS en puerto $Puerto (Modo Seguro)"
    Write-Host "======================================================"
    ...
    Write-Host "  [INFO] Asegurando servicios base (WAS y W3SVC)..." -ForegroundColor Cyan
    Set-Service WAS -StartupType Automatic
    Start-Service WAS -ErrorAction SilentlyContinue
    Set-Service W3SVC -StartupType Automatic
    Start-Service W3SVC -ErrorAction SilentlyContinue

    $siteName = "Practica7-IIS"
    $appPoolName = "Practica7-AppPool"
    $webroot  = "C:\inetpub\wwwroot\practica7"

    Write-Host "  [INFO] Limpiando configuraciones anteriores..." -ForegroundColor Cyan
    ...
    # --- CORRECCIÓN ERROR 401: Permisos para visitantes web (IUSR) ---
    $acl = Get-Acl $webroot
    ...
    Set-Acl -Path $webroot -AclObject $acl | Out-Null
    # -----------------------------------------------------------------

    Write-Host "  [INFO] Creando Application Pool '$appPoolName'..." -ForegroundColor Cyan
    New-WebAppPool -Name $appPoolName | Out-Null

    Write-Host "  [INFO] Creando sitio web '$siteName'..." -ForegroundColor Cyan
    New-Website -Name $siteName -Port $Puerto -PhysicalPath $webroot -ApplicationPool $appPoolName -Force | Out-Null

    Set-FirewallRule -Nombre "HTTP-IIS-$Puerto" -Puerto $Puerto
    New-IndexPage -Servicio "IIS" -Version "Nativa" -Puerto $Puerto -Webroot $webroot

    Write-Host "  [INFO] Iniciando Application Pool y Sitio Web..." -ForegroundColor Cyan
    ...
    $escucha = Get-NetTCPConnection -LocalPort $Puerto -State Listen -ErrorAction SilentlyContinue
    if ($escucha) {
        Write-Host "  [OK] IIS configurado y activo en puerto $Puerto." -ForegroundColor Green
    } else {
        Write-Host "  [WARN] IIS no logro escuchar. Diagnostico:" -ForegroundColor Yellow
        Get-Website -Name $siteName | Select-Object Name, State, Bindings | Format-Table -AutoSize
    }
}
...
function Enable-IISHttps {
    ...
    $cert = Get-ChildItem "Cert:\LocalMachine\My" | Where-Object { $_.FriendlyName -eq "Practica7-SSL" } | Select-Object -First 1
    ...
    if (-not (Get-Website -Name $siteName -ErrorAction SilentlyContinue)) {
         Write-Host "  [ERROR] El sitio '$siteName' no existe. Ejecuta la instalacion normal primero." -ForegroundColor Red
         return
    }

    Write-Host "  [INFO] Configurando binding HTTPS..." -ForegroundColor Cyan
    
    # 1. Eliminar cualquier binding HTTPS existente en el sitio
    Get-WebBinding -Name $siteName -Protocol "https" -ErrorAction SilentlyContinue | Remove-WebBinding -ErrorAction SilentlyContinue
    
    # 2. Crear el nuevo binding HTTPS
    New-WebBinding -Name $siteName -Protocol "https" -Port $Puerto -IPAddress "*" -SslFlags 0 | Out-Null

    # 3. Asociar certificado usando el Provider nativo de IIS (Método moderno y a prueba de fallos)
    $bindingPath = "IIS:\SslBindings\*!$Puerto"
    ...
    try {
        ...
        Write-Host "  [OK] Certificado SSL asociado correctamente." -ForegroundColor Green
    } catch {
        Write-Host "  [ERROR] Fallo al asociar el certificado: $_" -ForegroundColor Red
    }

    Set-FirewallRule -Nombre "HTTPS-IIS-$Puerto" -Puerto $Puerto

    $webroot = "C:\inetpub\wwwroot\practica7"
    New-IndexPage -Servicio "IIS" -Version "Nativa" -Puerto $Puerto -Webroot $webroot -Https $true

    Write-Host "  [INFO] Aplicando cambios en el sitio..." -ForegroundColor Cyan
    ...
    $escucha = Get-NetTCPConnection -LocalPort $Puerto -State Listen -ErrorAction SilentlyContinue
    if ($escucha) {
        Write-Host "  [OK] IIS HTTPS activo y escuchando en puerto $Puerto." -ForegroundColor Green
    } else {
        Write-Host "  [WARN] Puerto $Puerto no escucha." -ForegroundColor Yellow
    }
}